...
- Commit subject는 변경 대상과 목적이 드러나는 간결한 명사형 또는 작업명 중심으로 작성한다.
- Commit subject에는 기본적으로 `~한다`, `~했다`, `~합니다` 같은 문장형 종결을 사용하지 않는다.
- Issue 제목과 PR 제목은 작업 범위를 한눈에 알 수 있는 명사형 또는 작업명 중심으로 작성한다.
- Issue/PR 본문의 작업 목록과 체크리스트는 짧은 불릿과 완료 상태 중심으로 작성한다.
- Issue/PR의 배경과 설계 이유를 설명하는 문단은 의미 전달을 우선해 자연스러운 한국어 서술형으로 작성한다.
- 모든 문장을 억지로 명사형으로 바꾸지 않으며, 제목과 목록의 불필요한 `~한다` 반복만 줄인다.
...
- subject는 변경 내용을 요약하는 간결한 명사형 또는 작업명 중심으로 작성한다.
- subject에 기본적으로 문장형 종결(`~한다`, `~했다`, `~합니다`)을 사용하지 않는다.
- 72자를 크게 넘기지 않는다.
